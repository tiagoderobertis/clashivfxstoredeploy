...
export function TrainingPackCard({ setSkillLevel }: TrainingPackCardProps) {
  ...
  const features = [
    t("store.products.trainingPack.features.feature1"),
    t("store.products.trainingPack.features.feature2"),
    t("store.products.trainingPack.features.feature3"),
    t("store.products.trainingPack.features.feature4"),
    t("store.products.trainingPack.features.feature5"),
    t("store.products.trainingPack.features.feature6"),
  ]

  const handleBuyNow = () => {
    window.location.href = "https://e08ff1-xx.myshopify.com/products/formacion-1-a-1"
  }
    ...
    <div className="space-y-6">
      <div className="md:text-left text-center md:max-w-none max-w-[700px] mx-auto md:mx-0">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tighter sm:text-3xl text-cream">
          {t("store.products.trainingPack.title")}
        </h1>
        <p className="text-sm md:text-base text-cream/80 mt-2">{t("store.products.trainingPack.description")}</p>
      </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="font-medium ml-1">{t("store.skillLevel.training.title")}</span>
          ...
          <Card className="flex flex-col h-full bg-[#0A0A0A] border-orange-500/30">
            <CardHeader className="p-0">
              <div className="relative h-32 w-full">
                <div className="absolute top-2 right-2 bg-purple-600 text-xs font-bold text-white px-3 py-1 rounded-full z-10">
                  EXCLUSIVO
                </div>
                <div className="w-full h-full bg-gradient-to-r from-purple-600/20 to-indigo-700/20 rounded-t-lg flex items-center justify-center">
                  <h2 className="text-2xl font-bold text-cream">
                    {t("store.products.trainingPack.title").toUpperCase()}
                  </h2>
                </div>
              </div>
            </CardHeader>
              ...
              <div className="flex items-center justify-between w-full">
                <div className="flex flex-col">
                  ...
                  <div className="text-sm text-cream/60 line-through">
                    {currency === "ARS" ? "ARS $300.000" : "$300.00"}
                  </div>
                </div>
