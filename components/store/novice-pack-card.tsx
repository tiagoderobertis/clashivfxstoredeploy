...
interface NovicePackCardProps {
  setSkillLevel: (level: SkillLevel) => void
}

export function NovicePackCard({ setSkillLevel }: NovicePackCardProps) {
  ...
  const features = [
    t("store.products.novicePack.features.feature1"),
    t("store.products.novicePack.features.feature2"),
    t("store.products.novicePack.features.feature3"),
  ]

  const handleBuyNow = () => {
    window.location.href = "https://e08ff1-xx.myshopify.com/products/pack-principiantes"
  }
    ...
    <div className="space-y-6">
      <div className="md:text-left text-center md:max-w-none max-w-[700px] mx-auto md:mx-0">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tighter sm:text-3xl text-cream">
          {t("store.products.novicePack.title")}
        </h1>
        <p className="text-sm md:text-base text-cream/80 mt-2">{t("store.products.novicePack.description")}</p>
      </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="font-medium ml-1">{t("store.skillLevel.novice.title")}</span>
          ...
          <Card className="flex flex-col h-full bg-[#0A0A0A] border-orange-500/30">
            <CardHeader className="p-0">
              <div className="relative h-32 w-full">
                <div className="absolute top-2 right-2 bg-red-500 text-xs font-bold text-white px-3 py-1 rounded-full z-10">
                  50% OFF
                </div>
                <div className="w-full h-full bg-gradient-to-r from-blue-500/20 to-blue-700/20 rounded-t-lg flex items-center justify-center">
                  <h2 className="text-2xl font-bold text-cream">PACK PRINCIPIANTES</h2>
                </div>
              </div>
            </CardHeader>
            ...
            <CardFooter className="p-4 pt-0 mt-auto">
              <div className="flex items-center justify-between w-full">
                <div className="flex flex-col">
                  ...
                  <div className="text-sm text-cream/60 line-through">{getOriginalPrice()}</div>
                </div>
